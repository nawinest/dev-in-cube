...
export const cart = {
    ...
    effects: (dispatch) => ({
        async getCartAsync() {
            const res = await request.get('/cart/123456/items')
        ...
        async addCartItems(id) {
            ...
            try {
                const res = await request.post('/carts/123456/items', data, config)
                console.log(res.data)
            } catch (e) {
                console.log(e)
            }
        ...
        async getCartList() {
            const response = await request.get('/carts/123456/items')
